...
public class AllNodes {
    ...
    public static List<Class<? extends IISNode>> getDeclarationClasses() {
        return List.of(DeclareBooleanVariable.class,
                DeclareEquipmentTypeVariable.class,
                DeclareGrenadeTypeVariable.class,
                DeclareNumberVariable.class,
                DeclareObjectListVariable.class,
                DeclareObjectVariable.class,
                DeclareStringVariable.class,
                DeclareTeamVariable.class,
                DeclareUIMessageVariable.class,
                DeclareVector3Variable.class,
                DeclareVehicleTypeVariable.class,
                DeclareWeaponTypeVariable.class);
    }

    public static List<Class<? extends IVSONode>> getSetterNodes() {
        return List.of(SetBooleanVariable.class,
                SetEquipmentTypeVariable.class,
                SetGrenadeTypeVariable.class,
                SetNumberVariable.class,
                SetObjectListVariable.class,
                SetObjectVariable.class,
                SetStringVariable.class,
                SetTeamVariable.class,
                SetUIMessageVariable.class,
                SetVector3Variable.class,
                SetVehicleTypeVariable.class,
                SetWeaponTypeVariable.class
        );
    }

    public static List<Class<? extends ISONode>> getGetterNodes() {
        return List.of(GetBooleanVariable.class,
                GetEquipmentTypeVariable.class,
                GetGrenadeTypeVariable.class,
                GetNumberVariable.class,
                GetObjectListVariable.class,
                GetObjectVariable.class,
                GetStringVariable.class,
                GetTeamVariable.class,
                GetUIMessageVariable.class,
                GetVector3Variable.class,
                GetVehicleTypeVariable.class,
                GetWeaponTypeVariable.class
        );
    }

    public static boolean isObjectType(DataType type) {
        List<Class<? extends DataType>> objects = List.of(
                PlayerType.class,
                WeaponType.class,
                EquipmentType.class,
                VehicleType.class
        );
        if(objects.contains(type.getClass())){
            return true;
        }
        return false;
    }

    public static Class<? extends ISONode> getVariableGetterForType(DataType dataType) {
        if(isObjectType(dataType)){
            dataType = new ObjectType(dataType.getName());
        }
        for (Class<? extends ISONode> className : getGetterNodes()) {
            try {
                ISONode isoNode = (ISONode) (className.getConstructors()[0].newInstance());
                if (isoNode.getDataType().getClass() == dataType.getClass()) {
                    return className;
                }
    ...
    public static Class<? extends IVSONode> getVariableSetterForType(DataType dataType) {
        if(isObjectType(dataType)){
            dataType = new ObjectType(dataType.getName());
        }
        for (Class<? extends IVSONode> className : getSetterNodes()) {
            try {
                IVSONode isoNode = (IVSONode) (className.getConstructors()[0].newInstance());
                if (isoNode.getDataType().getClass() == dataType.getClass()) {
                    return className;
                }
            } catch (InstantiationException e) {
    ...
    public static Class<? extends IISNode> getVariableDeclarationForType(DataType dataType) {
        if(isObjectType(dataType)){
            dataType = new ObjectType(dataType.getName());
        }
        for (Class<? extends IISNode> className : getDeclarationClasses()) {
            try {
                IISNode isoNode = (IISNode) (className.getConstructors()[0].newInstance());
                if (isoNode.getDataType().getClass().getSimpleName().equals(dataType.getClass().getSimpleName())) {
                    return className;
                }
            } catch (InstantiationException e) {
                throw new RuntimeException(e);
